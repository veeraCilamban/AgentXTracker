...
export const AutoXEvalForm = ({ projectId, id }: AutoXEvalFormProps) => {
  const [selectedTrace, setSelectedTrace] = useState<{ trace: Trace } | null>(
    null,
  );
  ...
  const handleTraceSelection = (traceObj: { trace: Trace }) => {
    setSelectedTrace(traceObj);
  };
  ...
  const validateEvaluation = async () => {
    ...
    if (id === "autox-agent-quality" && !uploadedJson) {
    ...
    try {
      ...
      setError(
        `There was an error validating the evaluation: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    } finally {
  ...
  const executeEvaluation = async () => {
    ...
    try {
      ...
      setError(
        `There was an error executing the evaluation: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    } finally {
      setIsLoading(false);
    }
